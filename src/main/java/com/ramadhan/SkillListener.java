...
public class SkillListener implements Listener {
    ...
    // 🎨 RAMADAN GOLDEN MOON COLOR PALETTE
    private static final Color GOLD = Color.fromRGB(255, 215, 0);        // ✨ Emas suci
    private static final Color MOON_WHITE = Color.fromRGB(255, 250, 240); // 🌙 Putih bulan
    private static final Color CRESCENT_SILVER = Color.fromRGB(200, 200, 220); // 🌙 Perak crescent
    private static final Color STAR_SPARKLE = Color.fromRGB(255, 240, 180); // ⭐ Sparkle hangat
    ...
    public void onDamage(EntityDamageByEntityEvent e) {
        ...
        // ✅ Cooldown anti-spam (100ms = 10 hit/detik max)
        ...
        // 🌙 SKILL 2: Sneak + Hit = Crescent Dash
        ...
        // ⚔️ SKILL 1: Ramadan Combo (3 hit dengan animasi)
        data.combo++;
        if (data.combo == 2) {
            animateCombo2(p, target); // 🌙 Moon lift
        } else if (data.combo >= 3) {
            animateCombo3(p, target); // ⭐ Star burst
            data.combo = 0;
        }
    ...
    public void onPlayerDamage(EntityDamageEvent e) {
        ...
        // 🛡️ Shield passive: 10% DR + gentle sparkle on hit
        if (hasLunarShield(p)) {
    ...
    public void onMove(PlayerMoveEvent e) {
        ...
        // Charge animation (efficient: every 8 ticks = 0.4s)
    ...
    // ==========================================
    // 🌙 ANIMASI: COMBO 2 - MOON LIFT
    // ==========================================
    private void animateCombo2(Player p, LivingEntity target) {
        target.setVelocity(new Vector(0, 0.5, 0));
        target.damage(4.0, p);
        
        final Location start = target.getLocation().add(0, 1, 0);
        final World world = target.getWorld();
        
        // 🌙 Crescent arc animation (5 frames, 2 ticks apart = smooth)
        for (int frame = 0; frame < 5; frame++) {
            final int f = frame;
            new BukkitRunnable() {
                @Override
                public void run() {                    double angle = Math.toRadians(f * 30 - 60);
                    Vector offset = new Vector(Math.cos(angle) * 1.2, 0.3 + f * 0.2, Math.sin(angle) * 1.2);
                    Location particleLoc = start.clone().add(offset);
                    
                    // Crescent particles (elegant, not spam)
                    world.spawnParticle(Particle.DUST, particleLoc, 1, new Particle.DustOptions(CRESCENT_SILVER, 1.3f));
                    if (f == 2) world.spawnParticle(Particle.FLAME, particleLoc, 1, 0.1f, 0.1f, 0.1f, 0);
                }
    ...
    // ==========================================
    // ⭐ ANIMASI: COMBO 3 - STAR BURST
    // ==========================================
    private void animateCombo3(Player p, LivingEntity target) {
        target.damage(3.0, p);
        final Location center = target.getLocation().add(0, 1.2, 0);
        final World world = target.getWorld();
        
        // ⭐ Star burst: 8 directions, elegant sparkle
        for (int i = 0; i < 8; i++) {
            final int idx = i;
            new BukkitRunnable() {
                @Override
        ...
        // Center burst (delayed for impact feel)
        new BukkitRunnable() {
            @Override
            public void run() {
    ...
    private void animateCrescentDash(Player p, LivingEntity target) {
        ...
        // 🌙 Crescent trail animation (smooth, 6 frames)
        for (int frame = 0; frame < 6; frame++) {
            ...
            new BukkitRunnable() {
                ...
                public void run() {
                    ...
                    for (double angle = -40; angle <= 40; angle += 20) {
                        double rad = Math.toRadians(angle);
                        Vector arcOffset = new Vector(Math.cos(rad) * 0.6, 0, Math.sin(rad) * 0.6);
                        Location particleLoc = trailLoc.clone().add(arcOffset.rotateAroundY(Math.toRadians(90)));
        ...
        // Impact animation (delayed)
        new BukkitRunnable() {
            @Override
            public void run() {
                Location impactLoc = p.getLocation().add(0, 1, 0);
                
                // Crescent impact burst
                for (double angle = 0; angle < 360; angle += 30) {
                    double rad = Math.toRadians(angle);
                    Vector burstOffset = new Vector(Math.cos(rad) * 1.8, 0.3, Math.sin(rad) * 1.8);
                    Location particleLoc = impactLoc.clone().add(burstOffset);
                    world.spawnParticle(Particle.DUST, particleLoc, 1, new Particle.DustOptions(GOLD, 1.6f));
                }
                
                // Sound + damage                world.playSound(impactLoc, Sound.BLOCK_AMETHYST_BLOCK_HIT, 0.7f, 1.5f);
                target.damage(5.5, p);
                target.setVelocity(dir.clone().multiply(0.9).setY(0.5));
                
                // Small AOE (efficient: max 8 entities)
                int hitCount = 0;
                for (Entity en : world.getNearbyEntities(impactLoc, 2.8, 2.2, 2.8)) {
                    if (hitCount >= 8) break;
    ...
    // ==========================================
    // 🌕 ANIMASI: GOLDEN BLESSING (Ultimate)
    // ==========================================
    private void animateGoldenBlessing(Player p) {
        ...
        // 🕌 Title + sound (Ramadan vibe)
        p.playSound(center, Sound.BLOCK_AMETHYST_BLOCK_HIT, 0.9f, 1.0f);
        p.playSound(center, Sound.BLOCK_BEACON_ACTIVATE, 0.5f, 0.85f);
        p.sendTitle("§f§l🌕", "§6§lBerkah Bulan Suci", 4, 18, 6);
        
        // Phase 1: Crescent arena summon (elegant hexagon)
        for (int corner = 0; corner < 6; corner++) {
            ...
            new BukkitRunnable() {
                ...
                @Override
                public void run() {
                    if (frame >= 12) { this.cancel(); return; }
                    ...
                    Location cornerLoc = center.clone().add(
                        ...
                        0.25 + progress * 0.3,                         Math.sin(angle) * currentRadius
                    );
                    
                    // Crescent corner particles
        ...
        // Phase 2: Rising crescent blade (cinematic)
        new BukkitRunnable() {
            int frame = 0;
            @Override
            public void run() {
                if (frame >= 16) { 
                    // Phase 3: Blessing impact
                ...
                double y = frame * 0.55;
                Location bladeLoc = center.clone().add(0, y + 4, 0);
                
                // Crescent blade particles (elegant arc)
                for (double angle = -35; angle <= 35; angle += 10) {
                    ...
                    Location particleLoc = bladeLoc.clone().add(offset);
    ...
    private void executeBlessingImpact(Player p, Location center, World world, boolean isElite, double radius) {
        // 🌕 Soft flash (not blinding)        for (Player viewer : center.getWorld().getPlayers()) {
        ...
        // Gentle light pillars (Ramadan lantern vibe)
        for (int i = 0; i < (isElite ? 6 : 4); i++) {
            ...
            new BukkitRunnable() {
                ...
                public void run() {
                    ...
                    Location pillarLoc = center.clone().add(Math.cos(angle) * (radius * 0.7), 0, Math.sin(angle) * (radius * 0.7));
                    animateLightPillar(pillarLoc, world, isElite ? CRESCENT_SILVER : GOLD);
                }
            }.runTaskLater(plugin, i * 4);
        }
        
        // Ground blessing particles (flowing, gentle)
        new BukkitRunnable() {
            int pulse = 0;
            @Override
            public void run() {
                if (pulse >= 20) { this.cancel(); return; }
        ...
        // AOE blessing (gentle damage + heal vibe)
        ...
        // Self blessing (player heal + glow)
        try {
        ...
        // Short invulnerability + final sparkle
    ...
    private void animateLightPillar(Location loc, World world, Color color) {
        new BukkitRunnable() {
            int height = 0;
            @Override
            public void run() {
                if (height >= 14) { this.cancel(); return; }
                
                // Rising particle column
                Location pillarLoc = loc.clone().add(0, height, 0);
    ...
    // ==========================================
    // 🎨 HELPER: ELEGANT PARTICLE FUNCTIONS
    // ==========================================
    
    // ✨ Gentle sparkle: minimal particles, elegant
    private void gentleSparkle(Location loc, Color color, int count) {
    ...
    // 🌙 Gentle aura: pulsing ring effect
    private void gentleAura(Location loc, Color color, int rings) {
        World world = loc.getWorld();
    ...
    // 🌙 Start charge animation (elegant pulse)
    private void startChargeAnimation(Player p) {        new BukkitRunnable() {
            int pulse = 0;
            @Override
            public void run() {
                LunarPlayerData data = getData(p);
