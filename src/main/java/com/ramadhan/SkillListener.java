...
public class SkillListener implements Listener {
    ...
    public void onDamage(EntityDamageByEntityEvent e) {
        ...
        if (isBlockingWithShield(p)) { e.setCancelled(true); return; }
    ...
    public void onInteract(PlayerInteractEvent e) {
        ...
        if ((e.getAction() == Action.LEFT_CLICK_AIR || e.getAction() == Action.LEFT_CLICK_BLOCK) && hasShield && getData(p).isBlocking) {
            e.setCancelled(true);
            executeShieldBash(p);
        }
    }
    ...
    public void onMove(PlayerMoveEvent e) {
        ...
        if (data.isCharging) {
            ...
            if (data.chargeTicks > 100) { data.isCharging = false; data.chargeTicks = 0; sendActionBar(p, "§c✦ Charge cancelled"); }
        }
        if (armorManager.tryMoonStep(p)) { data.moonStepReady = false; new BukkitRunnable() { @Override public void run() { data.moonStepReady = true; }}.runTaskLater(plugin, 60); }
    }
    ...
    public void onSneak(PlayerToggleSneakEvent e) {
        ...
        LunarPlayerData data = getData(p);
        if (data.isBlocking) { data.isBlocking = false; data.lastBlockStart = 0; }
    }
    ...
    private void executeBasicStrike(Player p, LivingEntity target) {
        ...
        target.getWorld().playSound(target.getLocation(), Sound.BLOCK_AMETHYST_BLOCK_HIT, 0.4f, 1.8f);
    }
    ...
    private void executeComboSkill(Player p, LivingEntity target, int combo) {
        ...
        if (combo == 2) {
            world.playSound(p.getLocation(), Sound.BLOCK_AMETHYST_BLOCK_HIT, 0.5f, 1.4f);
            new BukkitRunnable() { @Override public void run() {
                ...
                try { finalTarget.addPotionEffect(new PotionEffect(PotionEffectType.SLOWNESS, 40, 0, false, false)); } catch(Exception ignored) {}
                ...
                world.spawnParticle(Particle.FLAME, impactLoc, 6, 0.2f, 0.2f, 0.2f, 0.05f);
                spawnCrescentCrater(impactLoc, world, 25);
                if (armorManager.hasCrescentChestplate(finalP)) {
                    try { finalP.addPotionEffect(new PotionEffect(PotionEffectType.ABSORPTION, 100, 0, false, false)); } catch(Exception ignored) {}
                    spawnLunarSpark(finalP.getLocation().add(0, 1.5f, 0), GOLD, 8);
                }
            }}.runTaskLater(plugin, 6L);
        } else if (combo == 3) {
            world.playSound(p.getLocation(), Sound.BLOCK_AMETHYST_BLOCK_HIT, 0.6f, 1.6f);
            Vector dir = p.getLocation().getDirection().setY(0).normalize();
            new BukkitRunnable() {
                int dashTicks = 0;
                final Vector dashVel = dir.clone().multiply(2.0);
                @Override public void run() {
                    if (dashTicks >= 12) { executeStarfallSlash(finalP, dir); this.cancel(); return; }
            ...
            new BukkitRunnable() { @Override public void run() { world.getEntities().forEach(en -> en.removeMetadata("starHit", plugin)); }}.runTaskLater(plugin, 50L);
        }
        getData(p).combo = 0;
    }
    ...
    private void executeStarfallSlash(Player p, Vector direction) {
        ...
        world.playSound(slashLoc, Sound.BLOCK_AMETHYST_BLOCK_HIT, 0.8f, 1.5f);
            ...
            if (en instanceof LivingEntity le && !en.equals(p)) {
                ...
                if (dot > 0.2) {
                    ...
                    if (armorManager.hasCrescentLeggings(p)) {
                        try { p.addPotionEffect(new PotionEffect(PotionEffectType.SPEED, 30, 1, false, false)); } catch(Exception ignored) {}
                    }
                }
            }
        });
        triggerScreenShake(p, 0.08f, 4);
    }
    ...
    private void executePerfectParry(Player p) {
        ...
        for(double angle = 0; angle < 360; angle += 20) {
            ...
            world.spawnParticle(Particle.DUST, parryLoc.clone().add(spread), 1, new Particle.DustOptions(MOON_WHITE, 1.5f));
        }
        world.playSound(parryLoc, Sound.BLOCK_AMETHYST_BLOCK_HIT, 1.0f, 1.8f);
        ...
        new BukkitRunnable() { @Override public void run() {
            ...
            new BukkitRunnable() { @Override public void run() { getData(p).parryBonus = false; }}.runTaskLater(plugin, 40L);
    ...
    private void executeShieldBash(Player p) {
        World world = p.getWorld();
        Vector dir = p.getLocation().getDirection().setY(0).normalize();
        world.spawnParticle(Particle.CLOUD, p.getLocation().add(0, 1, 0), 10, 0.5f, 0.3f, 0.5f, 0.1f);
        world.spawnParticle(Particle.DUST, p.getLocation().add(0, 1, 0), 15, new Particle.DustOptions(CRESCENT_SILVER, 2.0f));
        ...
        world.getNearbyEntities(p.getLocation(), 2.5, 2.0, 2.5).forEach(en -> {
            if (en instanceof LivingEntity le && !en.equals(p)) {
                ...
                try {
                    le.addPotionEffect(new PotionEffect(PotionEffectType.SLOWNESS, 20, 3, false, false));
    ...
    private void spawnShieldBarrier(Location loc, Color color, int count) {
        ...
        world.spawnParticle(Particle.FLAME, loc.clone().add(0, 0.8, 0), count / 2, 0.3f, 0.2f, 0.3f, 0.05f);
    }
    ...
    private void executeLunarEclipse(Player p) {
        ...
        p.playSound(center, Sound.BLOCK_AMETHYST_BLOCK_HIT, 1.2f, 1.0f);
        ...
        new BukkitRunnable() {
            int phase = 0;
            @Override public void run() {
                if (phase == 0) { spawnBlessingArena(center, world, isElite ? Color.fromRGB(100, 100, 150) : GOLD); world.playSound(center, Sound.BLOCK_END_PORTAL_SPAWN, 0.8f, 0.7f);
                } else if (phase == 1) { spawnRisingCrescent(center, world); world.playSound(center, Sound.BLOCK_AMETHYST_BLOCK_HIT, 1.0f, 1.2f);
                } else if (phase == 2) { world.spawnParticle(Particle.DUST, center.clone().add(0, 2, 0), 40, 3, 1.5, 3, 0.08f, new Particle.DustOptions(MOON_WHITE, 2.5f)); world.playSound(center, Sound.BLOCK_BEACON_AMBIENT, 0.9f, 0.8f);
                } else if (phase == 3) { executeBlessingImpact(finalP, center, world, isElite);
                } else { p.setInvulnerable(false); this.cancel(); return; }
                phase++;
            }
    ...
    private void executeBlessingImpact(Player p, Location center, World world, boolean isElite) {
        ...
        for(Player viewer : center.getWorld().getPlayers()) {
            if(viewer.getLocation().distance(center) < radius + 5) viewer.sendTitle("§f§l✦", "§b§lBlessing Descends", 3, 12, 5);
        }
        for(int i = 0; i < (isElite ? 8 : 5); i++) {
            final int delay = i * 5;
            new BukkitRunnable() { @Override public void run() {
                Location pillarLoc = center.clone().add((Math.random() - 0.5) * radius, 0, (Math.random() - 0.5) * radius);
                spawnLightPillar(pillarLoc, world, isElite ? CRESCENT_SILVER : GOLD);
                world.playSound(pillarLoc, Sound.BLOCK_AMETHYST_BLOCK_HIT, 0.6f, 1.3f);
            }}.runTaskLater(plugin, delay);
        }
        world.spawnParticle(Particle.DUST, center, 60, (int)(radius/2), 1, (int)(radius/2), 0.15f, new Particle.DustOptions(MOON_WHITE, 2.0f));
        world.spawnParticle(Particle.FLAME, center, 30, (int)(radius/3), 0.8f, (int)(radius/3), 0.1f);
        world.spawnParticle(Particle.CLOUD, center, 25, (int)(radius/2), 1.5f, (int)(radius/2), 0.2f);
        world.playSound(center, Sound.BLOCK_AMETHYST_BLOCK_HIT, 1.5f, 0.9f);
        ...
        world.getNearbyEntities(center, radius, radius, radius).forEach(en -> {
            if (en instanceof LivingEntity le && !en.equals(p)) {
                ...
                le.getWorld().spawnParticle(Particle.DUST, le.getLocation().add(0, 1, 0), 12, new Particle.DustOptions(MOON_WHITE, 1.5f));
                world.playSound(le.getLocation(), Sound.BLOCK_AMETHYST_BLOCK_HIT, 0.5f, 1.4f);
            }
        });
    ...
    private void spawnLightPillar(Location loc, World world, Color color) {
        new BukkitRunnable() {
            ...
            @Override public void run() {
                ...
                world.spawnParticle(Particle.DUST, loc.clone().add(0, height, 0), 3, 0.2f, 0.1f, 0.2f, 0, new Particle.DustOptions(color, 1.8f));
                world.spawnParticle(Particle.FLAME, loc.clone().add(0, height, 0), 1, 0.1f, 0.05f, 0.1f, 0);
                height++;
            }
        }.runTaskTimer(plugin, 0, 1);
    }
    ...
    private void spawnBlessingZone(Location center, World world, Player caster, int durationTicks, boolean isElite) {
        ...
        new BukkitRunnable() {
            int ticks = 0;
            @Override public void run() {
                if(ticks >= durationTicks) { world.spawnParticle(Particle.DUST, center, 25, 3, 1, 3, 0, new Particle.DustOptions(zoneColor, 1.8f)); this.cancel(); return; }
                for(int i = 0; i < 8; i++) {
                    Location particleLoc = center.clone().add((Math.random() - 0.5) * zoneRadius, 0.5 + Math.random() * 3, (Math.random() - 0.5) * zoneRadius);
                    world.spawnParticle(Particle.DUST, particleLoc, 1, new Particle.DustOptions(zoneColor, 1.2f));
                    world.spawnParticle(Particle.FLAME, particleLoc, 1, 0.1f, 0.1f, 0.1f, 0);
                }
                if(ticks % 20 == 0) {
                ...
                if(ticks % 5 == 0) {
                    for(double angle = 0; angle < 360; angle += 15) {
                        double rad = Math.toRadians(angle);
                        Location edge = center.clone().add(Math.cos(rad) * zoneRadius, 0.02, Math.sin(rad) * zoneRadius);
                        world.spawnParticle(Particle.DUST, edge, 1, new Particle.DustOptions(zoneColor, 1.5f));
                    }
                }
                ticks++;
            }
    ...
    private void spawnLunarSpark(Location loc, Color color, int count) {
        ...
        for(int i = 0; i < count; i++) {
            Vector spread = new Vector((Math.random() - 0.5) * 0.3, Math.random() * 0.4, (Math.random() - 0.5) * 0.3);
            world.spawnParticle(Particle.DUST, loc.clone().add(spread), 1, new Particle.DustOptions(color, 1.5f));
        }
    }
    ...
    private void spawnCrescentTrail(Location from, Vector direction, Color color, int density) {
        ...
        for(int i = 0; i < density; i++) {
            ...
            world.spawnParticle(Particle.DUST, particleLoc, 1, new Particle.DustOptions(color, 1.8f));
        }
    }
    ...
    private void spawnStarTrail(Location loc, Color color, int count) {
        World world = loc.getWorld();
        for(int i = 0; i < count; i++) {
            Vector spread = new Vector((Math.random() - 0.5) * 0.5, Math.random() * 0.6, (Math.random() - 0.5) * 0.5);
            world.spawnParticle(Particle.DUST, loc.clone().add(spread), 1, new Particle.DustOptions(color, 1.6f));
            world.spawnParticle(Particle.FLAME, loc.clone().add(spread), 1, 0.05f, 0.05f, 0.05f, 0);
        }
    }
                ...
                if(ticks % 10 == 0) {
                    world.getNearbyEntities(center, 3.0, 2.0, 3.0).forEach(en -> {
                        if(en instanceof LivingEntity le) {
                            try { if(!le.hasPotionEffect(PotionEffectType.SLOWNESS)) le.addPotionEffect(new PotionEffect(PotionEffectType.SLOWNESS, 20, 0, false, false)); } catch(Exception ignored) {}
                        }
                    });
                }
                ticks++;
        ...
        for(int corner = 0; corner < 6; corner++) {
            ...
            new BukkitRunnable() {
                ...
                @Override public void run() {
                    ...
                    Location cornerLoc = center.clone().add(Math.cos(angle) * 10, 0.3, Math.sin(angle) * 10);
                    world.spawnParticle(Particle.DUST, cornerLoc, 2, new Particle.DustOptions(color, 1.8f));
        ...
        new BukkitRunnable() {
            ...
            @Override public void run() {
                ...
                for(double angle = -45; angle <= 45; angle += 8) {
                    ...
                    world.spawnParticle(Particle.DUST, crescentCenter.clone().add(offset), 2, new Particle.DustOptions(GOLD, 2.0f));
                }
                world.spawnParticle(Particle.DUST, crescentCenter, 6, 0.4f, 0.8f, 0.4f, 0, new Particle.DustOptions(MOON_WHITE, 2.5f));
                frame++;
            }
        }.runTaskTimer(plugin, 0, 1);
    }
    ...
    private void triggerScreenShake(Player p, float intensity, int ticks) {
        ...
        new BukkitRunnable() {
            int count = 0;
            @Override public void run() {
                if(count >= ticks) { p.teleport(original); this.cancel(); return; }
        ...
        new BukkitRunnable() {
            ...
            @Override public void run() {
                ...
                for(double angle = 0; angle < 360; angle += 25) {
                    double rad = Math.toRadians(angle);
                    Location auraLoc = p.getLocation().add(Math.cos(rad) * radius, 0.6, Math.sin(rad) * radius);
                    p.getWorld().spawnParticle(Particle.DUST, auraLoc, 1, new Particle.DustOptions(GOLD, 1.8f));
                }
    ...
    private void updateChargeVisual(Player p, int chargeTicks) {
        if(chargeTicks % 12 == 0) {
            ...
            p.playSound(p.getLocation(), Sound.BLOCK_AMETHYST_BLOCK_HIT, 0.25f, Math.min(pitch, 2.0f));
        }
        if(chargeTicks >= 40) {
            p.getWorld().spawnParticle(Particle.DUST, p.getLocation().add(0, 1, 0), 4, new Particle.DustOptions(GOLD, 2.0f));
        }
    }
    ...
    private boolean isBlockingWithShield(Player p) { return hasLunarShield(p) && getData(p).isBlocking; }
    ...
    private static class LunarPlayerData {
        int combo = 0; long lastHitTime = 0; int lunarGauge = 0;
        boolean isCharging = false; int chargeTicks = 0;
        boolean isBlocking = false; long lastBlockStart = 0;
