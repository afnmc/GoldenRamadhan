...
public class SkillListener implements Listener {
    ...
    private boolean checkCD(Player p, String skill, int sec) {
        long last = cooldowns.getOrDefault(p.getUniqueId(), 0L); // Disesuaikan
        if (System.currentTimeMillis() - last < (sec * 1000L)) return false;
    ...
    private void executeDash(Player p) {
        ...
        p.getWorld().spawnParticle(Particle.EXPLOSION, p.getLocation(), 5); // Ganti EXPLOSION_LARGE ke EXPLOSION
        p.getWorld().getNearbyEntities(p.getLocation(), 5, 3, 5).forEach(en -> { // FIX: Pakai world.getNearbyEntities
    ...
    private void dropSword(Location center, Player p) {
        new BukkitRunnable() {
            ...
            public void run() {
                ...
                if (y <= 0) {
                    center.getWorld().spawnParticle(Particle.EXPLOSION, center, 1); // Ganti EXPLOSION_HUGE
                    center.getWorld().getNearbyEntities(center, 12, 5, 12).forEach(en -> { // FIX
